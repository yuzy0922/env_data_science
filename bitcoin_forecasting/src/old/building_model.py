...
def DNNC(train, valid, test, DV_colname, DV_shifts, IV_lags, metric, feature_selection):
    ...
    def create_DNNC(num_unit_input, num_layer_hidden, num_unit_hidden, activation, dropout_rate):
        model = keras.models.Sequential()
        model.add(keras.layers.Dense(num_unit_input, activation=activation, input_shape=(len(X_train_preprocessed.columns),))) # Input
    ...
    def objective(trial):
        ...
        num_unit_input = trial.suggest_int("num_unit_input", 32, 128)
        ...
        # Create the model
        model = create_DNNC(num_unit_input, num_layer_hidden, num_unit_hidden, activation, dropout_rate)
        # Compile the model
    ...
    # Create the model
    optimized_model.add(keras.layers.Dense(best_params["num_unit_input"], activation=best_params["activation"], input_shape=(len(X_train_preprocessed.columns),))) # Input
